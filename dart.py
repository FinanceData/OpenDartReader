...
class OpenDartReader():
    ...
    def list(self, corp, start=None, end=None, kind='', kind_detail='', final=True):
        ...
        return search.list(self.api_key, corp_code, start, end, kind, kind_detail, final)

    # 1-2. 공시정보 - 기업개황
    def company(self, corp):
        corp_code = self.find_corp_code(corp)
        return search.company(self.api_key, corp_code)
    ...
    def company_by_name(self, name):
        ...
        return search.company_by_name(self.api_key, corp_code_list)
    
    # 1-3. 공시정보 - 공시서류원본파일
    def document(self, rcp_no):
        return search.document(self.api_key, rcp_no)
    ...
    def report(self, corp, key_word, bsns_year, reprt_code='11011'):
        corp_code = self.find_corp_code(corp)
        return report.report(self.api_key, corp_code, key_word, bsns_year, reprt_code)

    # 3. 상장기업 재무정보
    def finstate(self, corp, bsns_year, reprt_code='11011'):
        corp_code = self.find_corp_code(corp)
        return finstate.finstate(self.api_key, corp_code, bsns_year, reprt_code)

    # 4-1. 지분공시 - 대량보유 상황보고
    def major_shareholders(self, corp):
        corp_code = self.find_corp_code(corp)
        return share.major_shareholders(self.api_key, corp_code)

    # 4-2. 지분공시 - 임원ㆍ주요주주 소유보고
    def major_shareholders_exec(self, corp):
        corp_code = self.find_corp_code(corp)
        return share.major_shareholders_exec(self.api_key, corp_code)
